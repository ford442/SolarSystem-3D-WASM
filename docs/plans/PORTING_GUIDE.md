...
alongside the TypeScript bindings it also emits — `SolarSystemCwrap` overloads,
`SolarSystemModule` `_Export` members, and `wasmBridge.exports.ts`) plus
`EXPORTED_RUNTIME_METHODS=['ccall','cwrap','HEAPF32']`.

**Embind (`--bind`) is not linked.** Conjunction and camera pose are assembled in
`web/src/wasmBridge.ts` (`getNextConjunction()`, `getCameraPose()`) from existing
scalars. Do not add more `GetFooX/Y/Z` keepalives. Revisit Embind when the API needs
to return planet lists or a settings snapshot — that is the documented trigger, not
every structured JS object.
